            ...
            for (usz k = 0; k < mesh->dim_z; ++k) {
                switch (mesh->kind) {
                    case MESH_KIND_CONSTANT:
                        mesh->cells.values[i * mesh->dim_y * mesh->dim_z + j * mesh->dim_z + k] = compute_core_pressure(
                    ...
                    case MESH_KIND_INPUT:
                        ...
                        {
                            mesh->cells.values[i * mesh->dim_y * mesh->dim_z + j * mesh->dim_z + k] = 1.0;
                        } else {
                            mesh->cells.values[i * mesh->dim_y * mesh->dim_z + j * mesh->dim_z + k] = 0.0;
                        }
                        break;
