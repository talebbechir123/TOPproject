    ...
    for (usz j = STENCIL_ORDER; j < dim_y - STENCIL_ORDER; ++j) {
        for (usz i = STENCIL_ORDER; i < dim_x - STENCIL_ORDER; ++i) {
            ...
            for (usz o = 1; o <= STENCIL_ORDER; ++o) {
                ...
                + A->cells[i][j][k - o].value * B->cells[i][j][k - o].value)/dpow(17.0, (f64)o);
            }
        }
    }
}
...
for (usz k = STENCIL_ORDER; k < dim_z - STENCIL_ORDER; ++k) {
 ...
 void solve_jacobi_CB(mesh_t* A, mesh_t const* B, mesh_t* C) {
    ...
    const int block_size_x = chunk_size; // Number of cells in x-direction per block
    const int block_size_y = chunk_size; // Number of cells in y-direction per block
    const int block_size_z = chunk_size; // Number of cells in z-direction per block
